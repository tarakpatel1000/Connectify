...
const Actions = ({ post }) => {
  ...
  const handleLikeAndUnlike = async () => {
    if (!user) {
      return toast("Error", "User is not logged in", "error");
    }
    if (isLiking) {
      return;
    }
  ...
  const handleReply = async () => {
    if(!user) {
      return toast("Error", "You must be logged in to reply a post", "error");
    }
    try {
      setIsReplying(true);
    ...
    <Flex flexDirection={"column"}>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <FormControl>
              <Input placeholder="Type here to reply..." value={reply} onChange={(e) => setReply(e.target.value)}/>
            </FormControl>
          </ModalBody>
